...
object_stream make_stream()
{
    return object_stream{make_dict(), stream{b("")}};
}
} // namespace

// ── construction ──────────────────────────────────────────────────────────────

TEST_CASE("object_stream construction", "[object_stream][construction]")
{
    SECTION("constructs with explicit dictionary and stream")
    {
        auto dict = make_dict();
        dict.set("Type", object{name{"Example"}});
        auto payload = b("payload");
        auto s = object_stream{std::move(dict), stream{payload}};
    ...
    {
        auto data = b("data");
        auto s = object_stream{make_dict(), stream{data}};
        REQUIRE(s.dictionary().get_integer("Length") == nullptr);
    }
    ...
    SECTION("constructed stream preserves existing Length from dictionary")
    {
        ...
        auto data = b("data");
        auto s = object_stream{std::move(dict), stream{data}};

        const auto* length = s.dictionary().get_integer("Length");
    ...
    {
        auto s = object_stream{make_dict(), stream{b("")}};
    ...
    SECTION("constructed stream preserves exact byte content")
    {
        ...
        auto s = object_stream{make_dict(), stream{input}};
...
TEST_CASE("object_stream dictionary access", "[object_stream][access]")
{
    SECTION("const dictionary access works")
    {
        auto dict = make_dict();
        dict.set("A", object{std::int64_t{1}});
        const auto s = object_stream{std::move(dict), stream{b("")}};

        REQUIRE(s.dictionary().size() == 1);
        const auto* a = s.dictionary().get_integer("A");
        REQUIRE(a != nullptr);
        REQUIRE(*a == 1);
    }

    SECTION("mutable dictionary access allows modification")
    {
        auto s = make_stream();
        s.dictionary().set("NewKey", object{name{"NewValue"}});
    ...
    SECTION("mutable dictionary access allows removing entries")
    {
        auto dict = make_dict();
        dict.set("RemoveMe", object{std::int64_t{99}});
        auto s = object_stream{std::move(dict), stream{b("")}};
...
TEST_CASE("object_stream stream access", "[object_stream][access]")
{
    SECTION("const stream access works")
    {
        auto data = b("const_data");
        const auto s = object_stream{make_dict(), stream{data}};
...
TEST_CASE("object_stream set_length", "[object_stream][length]")
{
...
TEST_CASE("object_stream sync_length", "[object_stream][length]")
{
    ...
    SECTION("sync length after direct stream mutation via mutable access")
    {
        auto s = make_stream();
        s.sync_length();
        REQUIRE(*s.dictionary().get_integer("Length") == 0);

        s.stream().data().push_back(static_cast<std::byte>('A'));
        REQUIRE(*s.dictionary().get_integer("Length") == 0); // not synced

        s.sync_length();
        REQUIRE(*s.dictionary().get_integer("Length") == 1);
    }
...
TEST_CASE("object_stream write happy path", "[object_stream][write]")
{
    ...
    SECTION("multiple writes accumulate and stay synced")
    {
        ...
        s.write(b2);
        REQUIRE(*s.dictionary().get_integer("Length") == 6);

        s.write(c);
        REQUIRE(*s.dictionary().get_integer("Length") == 9);
    ...
    SECTION("write after construction preserves append order")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 5);
    }
}

// ── write() edge / corrupt-style cases ────────────────────────────────────────

TEST_CASE("object_stream write edge cases", "[object_stream][write][edge]")
{
    SECTION("write after manual length tampering gets overwritten")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 4);
    }
    ...
    SECTION("write does not trigger sync on direct stream().write()")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 99); // unchanged
    }

    SECTION("write through object_stream does sync after direct stream write")
    {
        auto s = make_stream();
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 12);
    }
    ...
    SECTION("write preserves all byte values including nulls and 0xFF")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 256);
    }
    ...
    SECTION("write interleaved with set_length retains user intent on last call")
    {
        ...
        s.set_length(100);
        REQUIRE(*s.dictionary().get_integer("Length") == 100);

        auto w = b("world");
        s.write(w);
        REQUIRE(*s.dictionary().get_integer("Length") == 10); // write() re-syncs
    }

    SECTION("write when dictionary already holds non-integer Length key")
    {
        auto dict = make_dict();
        dict.set("Length", object{name{"NotAnInt"}});

        // Before write, Length is a name, not an integer
        REQUIRE(dict.get_integer("Length") == nullptr);
        const auto* name_val = dict.get_name("Length");
        REQUIRE(name_val != nullptr);
        REQUIRE(name_val->value == "NotAnInt");

        auto initial_data = b("abc");
        auto s = object_stream{std::move(dict), stream{initial_data}};
    ...
    SECTION("write after dictionary was mutated externally via reference")
    {
        auto s = make_stream();
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 10);
    ...
    SECTION("write large chunk then verify length")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == static_cast<std::int64_t>(chunk.size()));
    }
    ...
    SECTION("write many small chunks and verify length stays accurate")
    {
        ...
        for (std::size_t i = 1; i <= 200; ++i)
        {
            ...
            total += chunk.size();
            s.write(chunk);
            REQUIRE(*s.dictionary().get_integer("Length") == static_cast<std::int64_t>(total));
        }
    }
}
...
TEST_CASE("object_stream stream-dictionary interaction", "[object_stream][interaction]")
{
    SECTION("mutating stream via mutable data() then sync catches up")
    {
        ...
        REQUIRE(*s.dictionary().get_integer("Length") == 0); // still old value

        s.sync_length();
        REQUIRE(*s.dictionary().get_integer("Length") == 500);
    }
    ...
    SECTION("set_length to smaller value then write corrects it")
    {
        ...
        s.write(text);
        REQUIRE(*s.dictionary().get_integer("Length") == 17);
        ...
        auto more = b("more");
        s.write(more);
        REQUIRE(*s.dictionary().get_integer("Length") == 21);
    ...
    SECTION("sync then set_length to arbitrary value (user override)")
    {
        ...
        s.set_length(99999);
        REQUIRE(*s.dictionary().get_integer("Length") == 99999);
