...
namespace ripper::pdf::core
{
    cross_reference_section::cross_reference_section(
        ...
        : subsections_{std::move(subsections)},
          startxref_offset_{startxref_offset}
    {
    }
    ...
    const cross_reference_entry *cross_reference_section::find(std::uint32_t object_number) const noexcept
    {
    ...
    const cross_reference_entry *cross_reference_section::find(const indirect_reference &ref) const noexcept
    {
        return find(ref.object_number());
    }
    ...
    class indirect_object *cross_reference_section::commit(
    ...
    indirect_reference cross_reference_section::allocate(
        std::unique_ptr<class indirect_object> object) noexcept
    {
        const std::uint32_t number = next_object_number();
