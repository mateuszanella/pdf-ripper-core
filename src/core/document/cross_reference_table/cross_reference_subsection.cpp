...
{
    cross_reference_subsection::cross_reference_subsection(
        std::uint32_t first_object_number,
        entry_map entries) noexcept
        : first_object_number_{first_object_number},
    ...
    const cross_reference_entry *cross_reference_subsection::find(std::uint32_t object_number) const noexcept
    {
        auto it = entries_.find(object_number);
        if (it == entries_.end())
            return nullptr;
