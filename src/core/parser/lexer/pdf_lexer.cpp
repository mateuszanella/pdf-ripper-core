...
pdf_lexer::pdf_lexer(std::string_view content) : _content{content} {}
...
lexer_token pdf_lexer::read_token()
{
        ...
        while (_position < _content.size())
        {
            ...
            if (current == ')')
            {
                ...
                if (depth == 0)
                {
                    ...
                    return lexer_token{lexer_token_type::literal_string,
                                       _content.substr(start, end - start)};
                }
            }
        }

        throw parse_exception{
            "Unterminated literal string token while parsing string starting at offset " +
            std::to_string(start)};
    }
    ...
    if (is_number_start(ch))
    {
        ...
        return lexer_token{saw_dot ? lexer_token_type::real : lexer_token_type::integer, lexeme};
    }
...
{
    const auto token = next();
    if (token.type == lexer_token_type::eof || token.type == lexer_token_type::dictionary_end ||
    ...
    if (token.type == lexer_token_type::integer)
    {
        auto gen = peek();
        auto marker = peek(1);
        if (gen.type == lexer_token_type::integer && marker.type == lexer_token_type::keyword &&
            marker.lexeme == "R")
        {
            (void)next();
            (void)next();
